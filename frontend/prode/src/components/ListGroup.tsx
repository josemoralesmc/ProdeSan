...
async function getGroups(token: any) {
  const data = await groupsService.getGroups(token);
  return data.data;
}
const ListGroup = async (Token: any) => {
  const cookieStore = cookies();
  const token = cookieStore.get("Token");
  const res = await getGroups(token?.value);

  return (
    <div>
      {res.map((data: any) => (
        <Link href={`/groups/${data.GroupID}`}>
          <div key={data.GroupID} className="h-full flex justify-center">
            <div className="h-20 w-80  shadow-lg mb-4 ml-3 mr-3 bg-white/75 rounded-2xl flex content-center">
              ...
              <div className="w-full text-center">
                <h2 className="mt-6">{data.Name}</h2>
              </div>
            </div>
          </div>
        </Link>
